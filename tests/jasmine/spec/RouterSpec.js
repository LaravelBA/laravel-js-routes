describe("Router", function() {
  ...
  describe("When Router.route() it's called", function() {
    it('Should replace {params} in URI', function() {
    ...
    it('Should not replace wrong {params} in URI', function() {
      expect(Router.route('user.show', {wrong: 1})).toEqual('http://domain.tld/user/{id}?wrong=1');
    });

    it('Should return undefined for unexistent routes', function() {
    ...
    it('Should prevent many trailing slashes at the end', function() {
      expect(Router.route('home')).toEqual('http://domain.tld');
    });

    it('Should account for optional parameters of routes', function() {
      expect(Router.route('cart.add', {article: 'foo', count: 2})).toEqual('http://domain.tld/cart/add/foo/2');
    });

    it('Should ignore unused optional parameters of routes', function() {
